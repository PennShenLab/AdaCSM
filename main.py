...
def init_config():
    parser = argparse.ArgumentParser(description='AdaCSM (MoE Clustering Survival Machine)')
    ...
    parser.add_argument('--is_cluster', type=bool, default=True, help='whether to perform subtype clustering')
    ...
    parser.add_argument('--train_adacsm', default=True, type=bool, help='whether to train AdaCSM')
    parser.add_argument('--train_DCSM', dest='train_adacsm', type=bool, help=argparse.SUPPRESS)
...
if torch.cuda.is_available():
    ...
    print(f"Using CUDA device index {args.cuda_device}.")
elif torch.backends.mps.is_available():
    print("CUDA not available; using Apple MPS backend when supported by ops.")
else:
    print('CUDA/MPS not available to PyTorch: continuing on CPU.')
...
# for seed in [42]:
    ...
    if args.train_adacsm:
        print('-----------------------------train and test AdaCSM-------------------------------')
        model, c_index, pred_adacsm, pred_time_adacsm, rae_nc_adacsm, rae_c_adacsm \
            = train_test_DCSM(param, X_train, X_test, y_train, y_test, fix=True, method='AdaCSM',
        ...
        with open('models/ADACSM_{}_seed{}{}.pkl'.format(data_name, seed, model_suffix), 'wb') as file:
            pkl.dump(model, file)
    else:
        print('-----------------------------just test AdaCSM-------------------------------')
        with open('models/ADACSM_{}_seed{}_moe.pkl'.format(data_name, seed), 'rb') as file:
            model = pkl.load(file)
        c_index, pred_adacsm, pred_time_adacsm, rae_nc_adacsm, rae_c_adacsm = test_DCSM(model, X_test, y_test)

    print('AdaCSM_c_index on all test data: {:.4f}'.format(c_index))
    result_adacsm.append(c_index)
    print('AdaCSM_rae_nc on test data: {:.4f}'.format(rae_nc_adacsm))
    rae_nc_adacsm_list.append(rae_nc_adacsm)
    print('AdaCSM_rae_c on test data: {:.4f}'.format(rae_nc_adacsm))
    rae_c_adacsm_list.append(rae_c_adacsm)

    t_sample = sample_weibull(scales=pred_adacsm.squeeze(), shape=args.weibull_shape)
    cal_adacsm = calibration(predicted_samples=t_sample, t=t_test, d=e_test)
    print('AdaCSM_cal on test data: {:.4f}'.format(cal_adacsm))
    cal_adacsm_list.append(cal_adacsm)

    is_cluster = args.is_cluster
    if is_cluster:
        print('----------------------cluster data with AdaCSM------------------------')
        cluster_tags_adacsm, shape, scale = model.predict_phenotype(np.float64(X_test))
        shape = shape.detach().cpu().numpy()
        scale = scale.detach().cpu().numpy()
        X_test_list = []
        y_test_list = []
        for i in range(model.k):  # go through all the classes
            idx_i = np.where(cluster_tags_adacsm == i)[0]
            print('num in cluster {} is {}'.format(i, len(idx_i)))
            X_test_list.append(X_test[idx_i])
            y_test_list.append(y_test[idx_i])
        pval, logrank = plot_KM(y_test_list, 'AdaCSM', data_name, is_train=False,
                seed=seed, num_inst=args.num_inst, num_feat=args.num_feat,
                is_expert=False, shape=shape, scale=scale, t_horizon=t_test.max())
        logrank_adacsm.append(logrank)
        plot_Weibull_cdf(t_test.max(), shape, scale, data_name=data_name, seed=seed)

low_adacsm, high_adacsm = st.t.interval(0.95, df=len(result_adacsm)-1, loc=np.mean(result_adacsm), scale=st.sem(result_adacsm))
print('-----------------C Index results-----------------')
print('AdaCSM:{:.4f}±{:.4f} from {:.4f} to {:.4f}'.format(np.mean(result_adacsm), np.std(result_adacsm), low_adacsm, high_adacsm))
low_adacsm, high_adacsm = st.t.interval(0.95, df=len(logrank_adacsm)-1, loc=np.mean(logrank_adacsm), scale=st.sem(logrank_adacsm))
print('---------------logrank results-----------------')
print('AdaCSM:{:.4f}±{:.4f} from {:.4f} to {:.4f}'.format(np.mean(logrank_adacsm), np.std(logrank_adacsm), low_adacsm, high_adacsm))
print('---------------rae_nc results-----------------')
print('AdaCSM:{:.4f}±{:.4f}'.format(np.mean(rae_nc_adacsm_list), np.std(rae_nc_adacsm_list)))
print('---------------rae_c results-----------------')
print('AdaCSM:{:.4f}±{:.4f}'.format(np.mean(rae_c_adacsm_list), np.std(rae_c_adacsm_list)))
print('---------------cal results---------------------')
print('AdaCSM:{:.4f}±{:.4f}'.format(np.mean(cal_adacsm_list), np.std(cal_adacsm_list)))
